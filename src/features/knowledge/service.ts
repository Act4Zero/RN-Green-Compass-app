...
import { DAILY_DOSES, KNOWLEDGE_ITEMS, KNOWLEDGE_QUIZZES, KNOWLEDGE_TOPICS } from './data/catalog';
...
export const knowledgeService = {
  async getPublishedItems(locale = 'en'): Promise<KnowledgeItemDetail[]> {
    try {
      const { data, error } = await (supabase as any).from('published_knowledge_items').select('*').eq('locale', locale).limit(200);
  ...
  async searchKnowledge(query: string, filters: KnowledgeSearchFilters = {}, locale = 'en', cursor = 0, pageSize = 12) {
    ...
    let filtered = items.filter((item) => item.type !== 'daily');
  ...
  async getKnowledgeHome({ locale = 'en', userId, interests = [], activeCategories = [] }: { locale?: string; userId?: string; interests?: string[]; activeCategories?: string[] }): Promise<KnowledgeHomeData> {
    ...
    const summaries = items.filter((item) => item.type !== 'daily').map(toSummary);
    const dailyDose = DAILY_DOSES[Math.abs(new Date().getDate() - 1) % DAILY_DOSES.length];
    ...
    return {
      ...
      interactive: summaries.filter((item) => item.type === 'quiz').slice(0, 6),
  ...
  async getQuiz(itemId: string): Promise<PublicKnowledgeQuiz | null> {
    try {
    ...
    const quiz = KNOWLEDGE_QUIZZES.find((entry) => entry.itemId === itemId);
    if (!quiz) return null;
    return { ...quiz, questions: quiz.questions.map(({ correctOptionId: _correct, ...question }) => question) };
  },

  async submitQuizAttempt(userId: string | undefined, itemId: string, answers: Record<string, string>, eventId = createEventId()): Promise<QuizAttemptResult> {
    if (userId) {
      try {
    ...
    return { attemptId: eventId, score, passed: score >= quiz.passingScore, correctAnswers, totalQuestions: quiz.questions.length, feedback };
  },
  ...
  async verifyCertificate(code: string): Promise<CertificateVerification | null> {
    try {
      ...
      return { code: row.code, status: row.status, pathTitle: row.path_title, holderName: row.holder_name, issuedAt: row.issued_at, version: row.version, accreditationClaim: false };
    } catch {
      return null;
    }
  },
