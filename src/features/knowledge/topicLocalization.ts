...
export const KNOWLEDGE_TOPIC_BG: Record<string, { name: string; description: string }> = {
...
export function localizedTopic(topic: KnowledgeTopic, locale: KnowledgeLocale) {
  return locale === 'bg' && KNOWLEDGE_TOPIC_BG[topic.slug]
    ? KNOWLEDGE_TOPIC_BG[topic.slug]
