...
export default function MapPopup({ location }: { location: MapLocation }) {
  ...
  const localizeFeature = (label: string) => label === 'Upcoming event' ? t('Upcoming event', 'Предстоящо събитие') : label;
  const sourceLabel = location.source === 'Green Compass Community' ? t('Green Compass Community', 'Общност на Green Compass') : location.source;
  const licenceLabel = location.licence === 'Community event listing' ? t('Community event listing', 'Публикувано събитие на общността') : location.licence;
  ...
  return <View style={[theme.shadows.raised, { position: 'absolute', zIndex: 70, backgroundColor: theme.colors.backgroundElevated, borderWidth: 1, borderColor: theme.colors.border, overflow: 'hidden', ...(desktop ? { right: 20, top: 146, bottom: 54, width: 410, borderRadius: theme.radii.xl } : { left: 12, right: 12, bottom: 14, maxHeight: '68%', borderRadius: theme.radii.xl }) }]}>
    <ScrollView contentContainerStyle={{ padding: theme.spacing.lg, gap: theme.spacing.md }}>
      ...
      <View style={{ borderTopWidth: 1, borderTopColor: theme.colors.border, paddingTop: theme.spacing.sm, gap: 3 }}><Text style={[theme.typography.bodySmall, { color: theme.colors.textMuted, fontSize: 11 }]}>{t('Source', 'Източник')}: {sourceLabel || t('Green Compass curated catalogue', 'Подбран каталог на Green Compass')}</Text><Text style={[theme.typography.bodySmall, { color: theme.colors.textMuted, fontSize: 11 }]}>{t('Licence', 'Лиценз')}: {licenceLabel || t('Source-specific listing licence', 'Лиценз според източника')}</Text></View>
    </ScrollView>
  </View>;
}
