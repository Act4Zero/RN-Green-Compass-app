...
export default function MapLibreRenderer(props: MapRendererProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addLayer({ id: 'cycling-places', type: 'circle', source: CYCLING_PLACES_SOURCE, minzoom: 14, layout: { visibility: placeVisibility }, paint: { 'circle-color': ['match', ['get', 'kind'], 'parking', '#1951BE', '#087DAB'], 'circle-radius': 10, 'circle-stroke-color': '#FFFFFF', 'circle-stroke-width': 2 } });
      map.addLayer({ id: 'cycling-place-labels', type: 'symbol', source: CYCLING_PLACES_SOURCE, minzoom: 14, layout: { visibility: placeVisibility, 'text-font': ['Noto Sans Regular'], 'text-field': ['match', ['get', 'kind'], 'parking', 'P', '●'], 'text-size': 12, 'text-allow-overlap': true }, paint: { 'text-color': '#FFFFFF' } });
