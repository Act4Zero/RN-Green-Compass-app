...
export default function KnowledgeQuizScreen() {
  ...
  if (!item || !quiz) return <Screen><Content><StatePanel icon="help-circle-outline" title={t('Quiz unavailable', 'Тестът не е наличен')} message={t('This assessment may have been archived.', 'Този тест може да е архивиран.')} action={<AppButton label={t('Back to Hub', 'Назад към Центъра за знания')} onPress={() => router.replace('/knowledge' as any)} />} /></Content></Screen>;
