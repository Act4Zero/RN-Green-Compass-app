...
export default function DailyDoseScreen() {
  ...
  const dailyTypeLabel = (type: string) => type === 'daily_quote' ? t('QUOTE', 'ЦИТАТ') : t('TIP', 'СЪВЕТ');
  ...
  return <Screen><ScrollView showsVerticalScrollIndicator={false}><Content wide>
    ...
    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 18, marginTop: 26 }}>
      ...
      <Card style={{ flex: 1, minWidth: 280 }}><Text style={[theme.typography.h2, { color: theme.colors.text }]}>{t('Home-screen widget', 'Миниприложение за началния екран')}</Text><Text style={[theme.typography.bodySmall, { color: theme.colors.textMuted, marginTop: 6 }]}>{t('Choose the preferred native widget size. The shared daily schedule keeps app and widget in sync.', 'Изберете предпочитан размер. Общият дневен график синхронизира приложението и миниприложението.')}</Text><View style={{ flexDirection: 'row', gap: 8, marginTop: 16 }}><AppButton label={t('Small', 'Малък')} variant={preference.widgetSize === 'small' ? 'primary' : 'secondary'} onPress={() => void persist({ ...preference, widgetSize: 'small' })} /><AppButton label={t('Medium', 'Среден')} variant={preference.widgetSize === 'medium' ? 'primary' : 'secondary'} onPress={() => void persist({ ...preference, widgetSize: 'medium' })} /></View><Text style={[theme.typography.bodySmall, { color: theme.colors.textMuted, marginTop: 14 }]}>{t('Add it from your device widget gallery after installing the native app.', 'Добавете го от галерията с миниприложения, след като инсталирате приложението на устройството си.')}</Text></Card>
    </View>
