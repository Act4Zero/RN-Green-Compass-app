...
export default function KnowledgeQuestScreen() {
  ...
  if (!quest) return <Screen><Content><StatePanel icon="compass-outline" title={t('Quest unavailable', 'Мисията не е налична')} message={t('Choose another journey from the Hub.', 'Изберете друго пътешествие от Hub.')} /></Content></Screen>;
  ...
  const finish = async (nodeId: string) => { if (!user) return router.push('/auth/signin'); setBusy(nodeId); setError(null); try { setProgress(await knowledgeService.completeQuestNode(user.id, quest.id, nodeId)); } catch (cause) { setError(cause instanceof Error ? cause.message : t('This node could not be verified.', 'Тази точка не можа да бъде потвърдена.')); } finally { setBusy(null); } };
  return <Screen><ScrollView showsVerticalScrollIndicator={false}><Content>
    <PageHeader eyebrow={t('Branching Knowledge Quest', 'Разклонена Knowledge Quest')} title={quest.title[locale]} description={quest.summary[locale]} />
