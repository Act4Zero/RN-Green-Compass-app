...
export default function KnowledgeWebinarScreen() {
  ...
  const register = async () => {
    ...
    if (permission.status === 'granted' && reminderAt > new Date()) await Notifications.scheduleNotificationAsync({ content: { title: t('Green Compass live studio', 'Green Compass на живо'), body: locale === 'bg' ? `${webinar.speaker} започва след един час.` : `${webinar.speaker} starts in one hour.`, data: { url: `/knowledge/webinar/${webinar.id}` } }, trigger: { type: Notifications.SchedulableTriggerInputTypes.DATE, date: reminderAt } });
    setRegistered(true);
  };
  ...
  const submitQuestion = async () => {
    ...
    catch { setQuestionError(t('Question could not be submitted.', 'Въпросът не можа да бъде изпратен.')); }
