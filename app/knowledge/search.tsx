...
const TYPES: { label: string; value?: KnowledgeContentType }[] = [{ label: 'All' }, { label: 'Articles', value: 'article' }, { label: 'Guides', value: 'guide' }, { label: 'Videos', value: 'video' }, { label: 'DIY', value: 'diy' }, { label: 'Tours', value: 'tour' }, { label: 'Labs', value: 'simulation' }, { label: 'Quizzes', value: 'quiz' }, { label: 'Live', value: 'webinar' }, { label: 'Resources', value: 'resource' }];
...
function SearchResultRow({ item, last }: { item: KnowledgeItemSummary; last: boolean }) {
  ...
  const labels: Partial<Record<KnowledgeContentType, [string, string]>> = { article: ['Article', 'Статия'], guide: ['Guide', 'Ръководство'], video: ['Video', 'Видео'], quiz: ['Quiz', 'Тест'], resource: ['Resource', 'Ресурс'], diy: ['DIY', 'Направи си сам'], tour: ['Tour', 'Обиколка'], simulation: ['Lab', 'Лаборатория'], webinar: ['Live', 'На живо'] };
  const icons: Partial<Record<KnowledgeContentType, keyof typeof Ionicons.glyphMap>> = { article: 'document-text-outline', guide: 'list-outline', video: 'play-circle-outline', quiz: 'help-circle-outline', resource: 'bookmark-outline', diy: 'hammer-outline', tour: 'navigate-circle-outline', simulation: 'options-outline', webinar: 'videocam-outline' };
