...
export default function CommunityGroupDetailScreen() {
  ...
  const load = useCallback(async () => {
    ...
    catch { setDashboard(null); addNotification({ type: 'toast', severity: 'error', message: t('Unable to load group.', 'Групата не можа да бъде заредена.') }); }
    finally { setLoading(false); }
  }, [addNotification, id, t]);
  ...
  const setSharing = async () => {
    if (!dashboard) return;
    setBusy(true);
    try { await communityEngagementService.setSummarySharing(dashboard.group.id, !dashboard.group.shareSummary); await load(); addNotification({ type: 'toast', severity: 'success', message: dashboard.group.shareSummary ? t('Your comparison summary is private.', 'Обобщението за сравнение вече е лично.') : t('Your aggregate summary is now shared with this group.', 'Обобщените ви резултати вече се споделят с групата.') }); }
    catch { addNotification({ type: 'toast', severity: 'error', message: t('Unable to update sharing.', 'Настройката за споделяне не можа да бъде обновена.') }); }
  ...
  const createGoal = async () => {
    if (!dashboard) return;
    setBusy(true);
    try { await communityEngagementService.createGoal({ groupId: dashboard.group.id, title: goalTitle, description: goalDescription, metric, targetValue: Number(target), ...dates }); setGoalTitle(''); setGoalDescription(''); setShowGoalForm(false); await load(); addNotification({ type: 'toast', severity: 'success', message: t('Shared 30-day goal created.', 'Споделената 30-дневна цел е създадена.') }); }
    catch { addNotification({ type: 'toast', severity: 'error', message: t('Unable to create goal.', 'Целта не можа да бъде създадена.') }); }
    finally { setBusy(false); }
  };
  ...
  const contribute = async (goalId: string) => {
    ...
    setBusy(true);
    try { await communityEngagementService.addGoalContribution(goalId, value); setContributions((current) => ({ ...current, [goalId]: '' })); const nextDashboard = await communityEngagementService.getGroupDashboard(id!); setDashboard(nextDashboard); const completedGoal = nextDashboard.goals.find((goal) => goal.id === goalId && goal.status === 'completed'); addNotification({ type: completedGoal ? 'modal' : 'toast', title: completedGoal ? t('Shared goal completed!', 'Споделената цел е постигната!') : undefined, severity: 'success', message: completedGoal ? t('Your group reached the milestone together. Share the achievement and celebrate everyone’s contribution.', 'Групата постигна целта заедно. Споделете успеха и отпразнувайте приноса на всички.') : t('Your contribution is now part of the shared progress.', 'Приносът ви вече е част от общия напредък.'), action: completedGoal ? { label: t('Share milestone', 'Сподели постижението'), onPress: () => void Share.share({ message: t(`${nextDashboard.group.name} completed “${completedGoal.title}” in Green Compass — ${completedGoal.currentValue.toLocaleString()} ${metricLabel(completedGoal.metric)} together!`, `${nextDashboard.group.name} постигна „${completedGoal.title}“ в Green Compass — общо ${completedGoal.currentValue.toLocaleString()} ${metricLabel(completedGoal.metric)}!`) }) } : undefined }); }
    catch { addNotification({ type: 'toast', severity: 'error', message: t('Unable to add contribution.', 'Приносът не можа да бъде добавен.') }); }
    finally { setBusy(false); }
  };
