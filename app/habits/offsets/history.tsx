...
export default function OffsetHistoryScreen() {
  ...
  const load = useCallback(async () => { if (!user) return; setLoading(true); setError(null); try { setItems(await offsettingService.getOffsetHistory(user.id)); } catch { setError('Историята не можа да се зареди.'); } finally { setLoading(false); } }, [user]);
