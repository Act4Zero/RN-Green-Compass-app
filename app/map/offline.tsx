...
export default function OfflineMapsScreen() {
  ...
  <View style={{ gap: 12 }}>{packs.map((state) => {
    ...
    return <Card key={pack.id} elevated={state.status === 'ready'} style={{ gap: 10 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 12 }}><View style={{ flex: 1 }}><Text style={[theme.typography.h3, { color: theme.colors.text }]}>{pack.name[locale]}</Text><Text style={[theme.typography.bodySmall, { color: theme.colors.textMuted }]}>{formatBytes(pack.byteSize)} · zoom {pack.minZoom}–{pack.maxZoom} · {pack.version}</Text></View><Text style={[theme.typography.label, { color: state.status === 'ready' ? theme.colors.success : theme.colors.textMuted }]}>{state.status === 'ready' ? t('Ready', 'Готова') : state.status === 'update-available' ? t('Update', 'Обновяване') : ''}</Text></View>
      ...
      {!downloadable && state.status !== 'ready' ? <Text style={[theme.typography.bodySmall, { color: theme.colors.warning }]}>{t('Waiting for the signed production manifest.', 'Очаква подписания production manifest.')}</Text> : null}
